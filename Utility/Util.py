        ...
        try:
            ...
            for i in range(0,2):
                if i == 0:
                    ...
                    get_gold_data(to_request)
                    to_request.GET=QueryDict(f'db_choice={i}&frame=all&display=chart2&cache=false')
                    get_gold_data(to_request)
                else:
            ...
            get_currency_data(to_request)
            to_request.GET=QueryDict(f'frame=1y&cache=True&display=chart2&max=50')
            get_currency_data(to_request)
            ...
            get_currency_data(to_request)
            to_request.GET=QueryDict(f'frame=all&cache=True&display=chart2')
            get_currency_data(to_request)
            to_request = HttpRequest    
            to_request.method='GET'
            for i in range(1,8):

...
def check_goldusd(action='do'):
    try:
        # ดึงข้อมูลทั้งหมดเรียงตามเวลา (timestamp)
        all_data = GoldUS.objects.all().order_by('timestamp')
        ...
        # ดึงข้อมูลทั้งหมด
        all_records = GoldUS.objects.all()
            ...
            if ts in latest_ids:
                if record.id > latest_ids[ts]['id']:
                    ...
                    if action == 'do':
                        ...
                        GoldUS.objects.get(id=latest_ids[ts]['id']).delete()
        ...
        if action == 'show':
            result.update({
                ...
                "total_records": GoldUS.objects.count(),

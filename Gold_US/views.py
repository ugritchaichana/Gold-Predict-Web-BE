...
class CSVUploadView(APIView):
    ...
    def post(self, request):
        ...
        GoldPriceUS.objects.bulk_create(gold_price_entries)

        current_time = datetime.now().strftime('%H-%M-%d-%m-%y')
        output_dir = os.path.join(settings.BASE_DIR, 'Gold_US', 'csv-files')
        
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        
        output_file_path = os.path.join(output_dir, f'XAU-USD-Cleaned-({current_time}).csv')
        df.to_csv(output_file_path, index=False)

        client = storage.Client.from_service_account_json(settings.SERVICE_ACCOUNT_FILE)
        bucket = client.bucket(settings.GCS_BUCKET_NAME)
        output_file_name = f'XAU-USD-Cleaned-({current_time}).csv'
...
class TestAPIView(APIView):
    def get(self, request):
        return Response({"message": "Test API is working!"}, status=status.HTTP_200_OK)